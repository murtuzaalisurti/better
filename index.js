...
async function getSuggestions(
  ...
  try {
    ...
    if (message.refusal) {
      throw new Error(
        `the model refused to generate suggestions - ${message.refusal}`,
      );
    }
...
function log({ withTimestamp = true }) {
   ...
   * @returns {string}
   */
  const getLogText = (str) =>
    withTimestamp ? `[${new Date().toISOString()}]: ${str}` : str;
  return {
...
async function run() {
  ...
  try {
    ...
    const deleteExistingReviews = core.getInput(
      ...
      if (getBooleanValue(deleteExistingReviews)) {
        ...
        if (reviewsByBot.length > 0) {
          ...
          for (const review of reviewsByBot) {
            const reviewComments = await getAllCommentsUnderAReview(
              octokit,
              review.id,
            );
          ...
          info(
            "No reviews by bot found. Skipping deleting existing comments for all reviews by bot...",
          );
        }
      } else {
